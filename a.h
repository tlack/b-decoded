...
_ V nl(){w2("\n");}
_ S os(S s){R w2(s),nl(),s;} // output string
_ J oi(J j){R os(pi(j)),j;}  // long
_ F of(F f){R os(pf(f)),f;} // float
_ J ox(J j){R os(px(j)),j;} 
_ S sc(S s,I c){ // scans for c in null terminated string 
	W(*s-c)
		P(!*s++,0)   // return 0 if s is the terminator char
	R s;}           // otherwise, s points to where c is}
_ I scn(S s,I c,I n){ // returns the index in s containing c, max length n
	N(n,P(c==s[i],i))
	R n;}//_ K P1(J x){R(K)(X1<<48|x);}_ K P2(J x){R(K)(X2<<48|x);}_ K P3(J x){R(K)(X3<<48|x);}
_ K kK(I n){R tn(0,n);}
_ K kC(I n){R tn(KC,n);}
_ K kI(I n){R tn(KI,n);}
_ K kS(I n){R tn(KS,n);}
_ K pn(S s,I n){R memcpy(kC(n),s,n);} // return a copy of byte string s max length n
_ K kp(S s){R pn(s,strlen(s));}       // same as above, but deduce len with strlen 
