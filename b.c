...
R x=xy,KS<xu?(K)*L:KF==xu?kf(*M):ki((I)*L);}V1(lnk){}
K ps(S s){K z;I a=0;}
I ln(S s){I c=*s&31;R!*s?1:32>*s||4>c?2:15>*s?5:3;}I j(I t){R KF==t?32:0;}C B=2,RET=0,JJ[]={16,17,18,19,24,21,22,23};
...
K1(ev){
		x=xy,x=KS<xu?((K(*)())x)():KF==xu?kf(((F(*)())x)()):ki(((J(*)())x)());R x;
}
K ps(S s){
	K z;I a=0;
I A[]={0,7,6,2,1,8,9,10,11,3,12,13,14,15,5,4},B=5,RET=0xc3,
  JJ[]={0xeb,0x72,0x74,0x77,0xe9,0x73,0x75,0x76, 0x0f83};
// i believe these functions produce x86 bytecode
I m(I a,I b,I c){R 64*a+8*(7&b)+(7&c);}
K rex(I r,I x,I b,K y){R(r=7<A[r])+(x=7<A[x])+(b=7<A[b])?cj(0x40+4*r+2*x+b,y):y;}

K h(I o,I x,I y){
	R j2(256>o?c1(o):c2(o>>8,o),16>y?c1(m(3,x,y)):c5(m(0,x,5),y));}
...
K Jj(K x,I n){R cj(0x0f,c5(16+x[xn],n-4));}
K f3(I o,I x,I y){
 R 127>y
...
K o2(I t,I o,I r,I x,I y){

 K z;
 // P(a,b), if b, return a;
 P(KF==t,8u>y-8 ? AB("vex") :
	j2(c2(0xc5,16*(8&~r)+8*(15&~x)+(5-o?3:1)),h((C[]){0x10,0x58,0x5c,0x59,0x5e,0x2e,0,0,0x2a}[o],r,y)))

 I a=126<y,s;
 P(0<=o&&r==x&&(!a||3-o),4-o?f3(o,r,y):129-y?AB("/"):i(0xd1,16+7,r))
 P(0<o&&r==y,z=f3(o,r,x),2-o?z:j2(z,i(0xf7,16+3,r)))
 P((a?3:1)<o,j2(f3(0,r,x),f3(o,r,y)))
 R s=0<o
  ? 0
	: 3+(o+1)/2,rex(r,a?0:y,x,c3(0>o?1&o?0x8b:0x89:3-o?0x8d:0x6b,m(3-o?a:3,A[r],a?A[x]:4),a?(2-o?y-128:128-y)<<s:m(s,A[y],A[x])));
}
...
I ln(S s){
	I o=*s++,
	  h=o/16,
		p=0xc5==o?2:0x0f==o;
	R 4==h?1+ln(s):RET==o||5==h?1:*JJ==o||7==h?2:0xe==h||0xb==h?5:p&&8==*s/16?6:p+(3==s[p]/64?2+(0x83==o||0x6b==o):5==(0xc7&s[p])?6:3);
}
...
K O2(I t,I f,I r,K x,K y){
	I i=Ay?yi:yu;
	R u(r,j3(Ay?c0():y,x,10>f?o2(t,f,r,xu,i):j2(cm(t,xu,i),16>r?cc(f-9,r):c1(f-9))));
}
...
C N=8,D[2]={1,1},L[26],T[26];

// N(n,a) performs a n times, providing i as a counter
N(26,L[i]=T[i]=0)

S r='['==s[1]&&(r=sc(s,']'))&&*++r?r:0;

I M=0;K*k=r||':'==s[1]?a=*s,s+=2,G+a-'a':0;P('!'==*s,++s,X(k,enm(ki(ip(s,strlen(s))))))

// I1(name) defines a monadic function (ie one arg)
// here are t and q:
I1(t){
	I a=xi-'a';
...
K g(I c,K x){
 ...
 W(++i<xn){
	 z=Xx;
	 I l=M&1<<i,b=Az||128>*z&&':'-*z||i-L[I(zy)-'a'],
	   h=2-i||Az||26>c;
	 z=f(i,z);
...
K d(I r,K x){
 ...
 I1(h){I t=T[xi-'a'];R 14==t?-2:13==t?-4:2*t-26;}
 I s=15&r,a;
 K y,z;
 S((y=xy,c(a=*x)),
  case'N':
  C('W',R w(x))
	...
	C('{',R E(r,x))
	C('[',R g(26,x))
	C('a',R T[a-'a'] ? O2(0,1+h(xx),s,d(0,xx),d(0,y)) : g(a-'a',x))
	C(0,  R y=d(0,y),O2(t(x),U(a-128),yu,y,d(s,xz))),
 if(':'==a){
	P(Ay,r=L[yi-'a'],M&=~(1<<r),f(r,xz))y=d(0,yy),z=e(s,xz),x=xy;R r=zu,u(r,j2(z,O2(0,h(xx),r,d(0,xx),y)));
 } 
 I m,b=t(y);
 ...
 a=U(a),
 r=9<a||16-r?r:0;
 P(!Ay&&!q(y)&&!Az&&!q(y),M|=1<<(m=D[KF==b]++),y=e(0,y),M&=~(1<<m),z=O2(b,a,r,y,f(m,z)),--D[KF==b],z)
 R Ay && !q(y) && 2-a && 4-a 
	? O2(b,9<a?11-a+11:a,r,e(s,z),y)
...
K p(){
	...
	K1(n){
		R kc(KI==Ax&&129u>1+xi?128+xi:(z=jk(z,x),16+zn-3));
	}
	K E(I a,I c){
		K r=k1(kc(c)),x;
		do 
 ...
 S('0'-c('-'==(a=*s++)?s['.'==*s]:'.'==a?*s:a)?c(a):'0',
  ...
  C('a', x='['==*s 
		? ++s,E(T[b=a-'a']?T[b]-8:(x=G[b],x=xy,*D=MX(*D,x[xn-2]),D[1]=MX(D[1],x[xn-1]),xu),a)
		: kc(a)),
	AB(s-1))
 P(q(),x)
 if('+'-c(a=*s++))AB(s-1);
...
{I a=t(x);lnk(zy=u(a,j2(X0(Ax||'$'-*x?f(0,x):v(0,x,0)),c3(RET,*D,D[1]))));} //dis(zy);
